    ...
    for ( std::shared_ptr<const Object> object : dataModelObjects )
    {
        ...
        for ( auto it = classInheritanceStack.begin(); it != classInheritanceStack.end(); ++it )
        {
            ...
            if ( classKeyword == object->classKeyword() )
            {
                ...
                {
                    auto scriptability = field->template capability<AbstractFieldScriptingCapability>();
                    if ( scriptability != nullptr )
                    {
                        ...
                        if ( pdmValueField )
                        {
                            ...
                            auto proxyField     = dynamic_cast<const ProxyFieldHandle*>( field );
                            ...
                            QVariant valueVariant = pdmValueField->toVariant();
