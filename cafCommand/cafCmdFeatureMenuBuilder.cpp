...
///
//--------------------------------------------------------------------------------------------------
void CmdFeatureMenuBuilder::appendToMenu( MenuInterface* menu )
{
    CAF_ASSERT( menu );

    std::vector<MenuInterface*> menus = {menu};
    for ( size_t i = 0; i < m_items.size(); i++ )
    {
        ...
        {
            MenuInterface* subMenu = menus.back()->addMenu( m_items[i].icon, m_items[i].itemName );
        ...
        else if ( m_items[i].itemType == MenuItem::SUBMENU_END )
        {
            if ( menus.size() > 1 )
            {
                MenuInterface* completeSubMenu = menus.back();
                menus.pop_back();

                if ( !menus.empty() )
                {
                    // Remove the sub menu action if no (sub) actions are present in the sub menu
                    if ( completeSubMenu->actions().empty() )
                    {
                        MenuInterface* menuWithEmptySubMenu = menus.back();

                        std::shared_ptr<ActionWrapper> subMenuAction = completeSubMenu->menuAction();
        ...
        else
        {
            ...
            if ( feature->canFeatureBeExecuted() )
            {
                ...
                auto existingActions = currentMenu->actions();
                for ( std::shared_ptr<ActionWrapper> existingAction : existingActions )
                {
                    // If action exist, continue to make sure the action is positioned at the first
                    // location of a command ID
                    if ( existingAction->isEqualTo( act.get() ) )
                    {
                        duplicateAct = true;
                        break;
                    }
