...
def get_all_artifacts(repo_name: str, headers: dict) -> []:
    ...
    ref = "https://api.github.com/repos/" + org_name + "/" + repo_name + \
        "/actions/artifacts" + "?per_page=" + str(amount_items_per_page)
    ...
    try:
        res = urllib.request.urlopen(urllib.request.Request(
            url="https://api.github.com/repos/" + org_name + "/" +
                repo_name + "/actions/artifacts/" + artifact_id,
            headers=headers,
            method='DELETE')
        ).getcode()
        return res == 204
...
print("Amount of artifacts for repo " +
      repo_name + " is " + str(len(artifacts)))
...
if len(artifacts) > keep_artifacts:
    for a in artifacts[keep_artifacts:]:
        ...
        if deleted:
            print("Repo: " + repo_name + " | Artifact with id " +
                  artifact_id + " has been deleted")
        else:
            print("Repo: " + repo_name +
                  " | Something went wrong while deleting artifact with id " + artifact_id)
