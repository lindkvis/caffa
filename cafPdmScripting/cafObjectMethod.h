...
public:
    ...
    virtual QString selfClassKeyword() const { return m_self->capability<ObjectIoCapability>()->classKeyword(); }
...
public:
    ...
    std::shared_ptr<ObjectMethod> createMethod( ObjectHandle* self, const QString& methodName );

    template <typename ObjectDerivative, typename ObjectScriptMethodDerivative>
    bool registerMethod()
    {
        QString className  = ObjectDerivative::classKeywordStatic();
        QString methodName = ObjectScriptMethodDerivative::classKeywordStatic();
...
private:
    ...
    std::map<QString, std::map<QString, std::shared_ptr<ObjectMethodCreatorBase>>> m_factoryMap;
    // Self pointer
